...
**Product capability ≠ MCP tool count.** Product default is `OCCAM_PROFILE=reader` (11 tools).
"18 tools" is the `OCCAM_PROFILE=full` stdio `tools/list` with opt-in flags off — one exposure
