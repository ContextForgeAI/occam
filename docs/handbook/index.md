...
**Version baseline:** 1.0.0 (published install channel) · core MCP tools from the registry (runtime `tools/list` varies by profile/opt-in; product default `OCCAM_PROFILE=reader`) · live extract by default (opt-in `cache_ttl_s` for local replay)
