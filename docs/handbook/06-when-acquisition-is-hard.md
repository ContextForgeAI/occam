...
Every wall maps to a typed `failure.code` and at most one honest lever: real cookies/session, proxy, forced browser backend, or operator-configured managed provider. There is no CAPTCHA solver, no identity rotation, no universal bypass.
...
| CAPTCHA / bot challenge | `captcha_or_challenge` | **None in product** — operator must use human session or accept failure |
| Geo / IP / bot blocking | `http_403`, thin extract | Session, proxy list, browser, managed provider |
...
- `OCCAM_HTTP_PROXY` / `OCCAM_HTTPS_PROXY` reach **worker egress**, not all Core `HttpClient`s (probe, map, managed, search may ignore them).
