...
- **`1.0.0` (GA; public default):** self-contained archives (`runtimeLayout=self-contained-v1`) with SHA-256 plus Cosign when `signaturePolicy=required-cosign-v1` (requires the `cosign` CLI). Executable helper overlays from mutable `main` are not part of that runtime contract.
- **npm `ff-occam`:** Published experimental npm package; it wraps the lower-level `@ff-occam/mcp` runtime. Do not present npm as the guarded GA install path.
...
- npm is a public experimental channel, not the guarded GA path.
