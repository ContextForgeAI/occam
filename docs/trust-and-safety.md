...
| Telemetry / phone-home endpoint? | No remote telemetry endpoint. `occam status` queries GitHub Releases for a hint; `occam update` compares `VERSION` to the latest release and only downloads when an upgrade (or `--force`) is needed. Normal extracts do not check for updates |
