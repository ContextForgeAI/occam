...
| `workers_unavailable` | `OCCAM_HOME` unset or workers missing | Set `OCCAM_HOME` to install/repo root; run `occam doctor` or `occam install-workers` |
| `ERR_MODULE_NOT_FOUND` / `@mozilla/readability` (worker crash) | `workers/node_modules` empty or corrupt (doctor used to skip install when the directory merely existed) | `occam install-workers --force` (or `OCCAM_WORKERS_FORCE_INSTALL=1` + `occam doctor`). Verify: `node scripts/lib/ensure-workers-deps.mjs --check` → `WORKERS_DEPS_OK` |
| `occam` is not recognized after `npm install -g ff-occam` | npm does not install the operator CLI; on Windows the npm global bin is often missing from PATH | Do not run `occam connect` from npm. Use the bootstrap in [Install](install.md). MCP-only trial: `npx ff-occam@1.2.0` |
| `occam connect` / `doctor` via `npx ff-occam` | npm package only starts the MCP host | Use guarded install (`get-ff-occam`), then `occam connect` — see [Install](install.md) |
...
| `requires_login` / `http_403` | Gated content with no usable document | `occam-session.mjs export-state` → `session_profile`. A 401/403 that still yields a real article is `ok:true` with `access.disposition=blocked-but-content-available` — not these failures |
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| `search_unconfigured` | Provider off / incomplete config | Default is DuckDuckGo when unset; set `OCCAM_SEARCH_PROVIDER=off` to disable, configure searxng/brave/tavily, or use `OCCAM_SEARCH_PROVIDERS` for fan-out |
| `search_rate_limited` | Local rate window / all arms degraded | Wait `OCCAM_SEARCH_DEGRADE_MINUTES` or add another backend in `OCCAM_SEARCH_PROVIDERS` |
