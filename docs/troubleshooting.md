...
| `occam` is not recognized after `npm install -g ff-occam` | npm does not install the operator CLI; on Windows the npm global bin is often missing from PATH | Do not run `occam connect` from npm. Use the bootstrap in [Install](install.md). MCP-only trial: `npx ff-occam@1.1.1` |
