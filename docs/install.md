...
6. Verifies the Occam host by required tool identity — default `reader` exposes **11** core tools; `full` exposes **18**
...
| `OCCAM_VERSION` | `1.3.0` (public default) | Release version; set an older tag only for a legacy channel |
