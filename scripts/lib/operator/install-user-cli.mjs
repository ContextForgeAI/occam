...
export function writeUserLauncher(binDir, occamHome, opts = {}) {
  ...
  try {
    ...
    // best-effort — launcher still embeds absolute node
  }
  if (plat === "win32") {
    const cmdPath = join(binDir, "occam.cmd");
    const ps1Path = join(binDir, "occam.ps1");
    writeFileSync(cmdPath, renderWindowsCmdLauncher(occamHome, nodeBin), "utf8");
    writeFileSync(ps1Path, renderWindowsPs1Launcher(occamHome, nodeBin), "utf8");
  ...
  writeFileSync(shPath, renderUnixLauncher(occamHome, nodeBin), "utf8");
  try {
