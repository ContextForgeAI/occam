...
**Version:** `1.3.0` (eighteen tools + opt-in batch/watch/crosscheck/browser_interact; Agent-First AF-1..AF-6; Receipt v1; live-only). Public install default: published `1.3.0`.
...
3. **Backend transparency** — success includes the winning extractor identifier (for example `node_readability_turndown`, `browser_playwright`, `npm_registry_package`, `crates_io_readme`, or `pdf`). This is not the request's abstract `backend_policy`.
...
- MCP `isError:true` on a typed envelope is **not** an empty failure. Parse
  `content[].text` / `structuredContent` first (`ok:false` + `failure.code`).
  Dropping that body and treating the call as an untyped bug is a client error.
...
| `backend` | string | Winning extractor identifier, such as `node_readability_turndown`, `browser_playwright`, `npm_registry_package`, `crates_io_readme`, `pdf`, or `css_extract_http`. Do not compare this field to `backend_policy`. |
  ...
  "items": [
    {
      ...
      "receipt": {
        ...
        "signed": {
          ...
          "ts": "2026-07-04T12:00:00Z", "toolchain": "ff-occam/1.0.0",
...
```json
{
  ...
  "receipt": {
    ...
    "signed": {
      ...
      "backend": "http", "ts": "2026-07-04T12:00:00Z", "toolchain": "ff-occam/1.0.0",
...
`compile.omitted`: a structured manifest of filtering or budget losses. Filtering can populate it while `compile.truncated` remains false. Fields: `reason` (budget strategy, `focus_filtered`, or `focus_filtered_and_budget`), `tokensDropped` (nonnegative net estimated size reduction, including marker overhead), `regions` (array of `tail` / `middle` / `unchosen` / `filtered` / `structured`), `sections` (count of top-level markdown sections that vanished; omitted when none), and optional `structured` counts (`blocks`, `tables`, `chunks`, `media`, `feedItems`, `screenshot`) when sidecar trim ran. In-band SNIP comments count removed body blocks within retained sections too, when marker space permits; the manifest does not depend on parsing those comments.

`completeness` evaluates retention of the selected answer unit, not the whole source. Locally coupled instruction blocks are protected before optional context; an oversized selected instruction is omitted explicitly rather than returned as a dangling introduction. `focus_body_filtered` indicates a missing answer without budget truncation and carries no budget retry suggestion. `focus_body_truncated` indicates budget loss; `suggestedMinTokens`, when present, is an estimate in caller budget units above the current budget, not a guarantee of full-page retention. Digest `focusMatched` is lexical evidence and can be true with structural `focus.status:weak`; neither implies completeness.
...
| `OCCAM_NODE_BIN` | Explicit `node` path (advanced). Launcher stamps from `{OCCAM_HOME}/runtime/node-bin` or `process.execPath` when unset — do not put this in MCP host configs. |
