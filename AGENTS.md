...
**Live by default** — every call fetches the page unless the caller explicitly opts into the local TTL-bound response cache with `cache_ttl_s > 0`; private/session-bound requests are never cached (**v1.3.0** published — **18** core tools + Receipt v1 + opt-in batch/watch/consensus/atlas/exam; Agent-First AF-1..AF-6; PB2–PB4b shipped; L0 core CLOSED).
